 ...
 */
export function packetToNormalisedHSBK(packetColor: ColorHSBK): ColorHSBK {
 ...
 */
export function normalisedToPacketHBSK(hsbkColor: ColorHSBK): ColorHSBK {
 ...
 */
export function validateNormalisedColorHSB(hue: number, saturation: number, brightness: number) {
	if (hue < HSBK_MINIMUM_HUE || hue > HSBK_MAXIMUM_HUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Light expects hue to be a number between ' + HSBK_MINIMUM_HUE + ' and ' + HSBK_MAXIMUM_HUE
			)
			.build();
	}
	...
	if (saturation < HSBK_MINIMUM_SATURATION || saturation > HSBK_MAXIMUM_SATURATION) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Light expects saturation to be a number between ' +
	...
	if (brightness < HSBK_MINIMUM_BRIGHTNESS || brightness > HSBK_MAXIMUM_BRIGHTNESS) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Light expects brightness to be a number between ' +
 ...
 */
export function validateNormalisedColorHSBK(hue: number, saturation: number, brightness: number, kelvin: number) {
 ...
 * @param brightness brightness value to validate
 */
export function validateRawColorHSB(hue: number, saturation: number, brightness: number) {
 ...
 * @param kelvin temperature to validate
 */
export function validateRawColorHSBK(hue: number, saturation: number, brightness: number, kelvin: number) {
