import { PowerRequest, POWER_MINIMUM_RAW, POWER_MAXIMUM_RAW } from './power';
...
function toBuffer(obj: PowerRequest) {
	...
	if (obj.power !== POWER_MINIMUM_RAW && obj.power !== POWER_MAXIMUM_RAW) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				`Invalid level given for setPower LIFX packet, only ${POWER_MINIMUM_RAW} and ${POWER_MAXIMUM_RAW} are supported`
			)
			.build();
	}
...
export const setPowerLegacy: PacketBodyHandler = {
	type: 21,
	name: 'setPowerLegacy',
	legacy: true,
	size: SIZE,
