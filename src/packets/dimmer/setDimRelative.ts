import { DimRelativeRequest } from './dimmer';
...
function toBuffer(obj: DimRelativeRequest) {
	...
	if (obj.brightness !== HSBK_MINIMUM_RAW && obj.brightness !== HSBK_MAXIMUM_RAW) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Light expects brightness to be ' + HSBK_MINIMUM_RAW + ' or ' + HSBK_MAXIMUM_RAW + ' only'
			)
			.build();
	}
...
export const setDimRelative: PacketBodyHandler = {
	type: 105,
	name: 'setDimRelative',
	legacy: false,
	size: SIZE,
