...
function toBuffer(obj: ColorZonesRequest) {
	...
	if (obj.startIndex < ZONE_INDEX_MINIMUM_VALUE || obj.startIndex > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid startIndex value given for getColorZones LIFX packet, must be a number between ' +
	...
	if (obj.endIndex < ZONE_INDEX_MINIMUM_VALUE || obj.endIndex > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid endIndex value given for getColorZones LIFX packet, must be a number between ' +
					ZONE_INDEX_MINIMUM_VALUE +
					' and ' +
					ZONE_INDEX_MAXIMUM_VALUE
			)
...
export const getColorZone: PacketBodyHandler = {
	type: 502,
	name: 'getColorZone',
	legacy: false,
	size: SIZE,
