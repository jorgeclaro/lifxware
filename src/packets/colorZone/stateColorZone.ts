...
import { ColorZone, ZONE_INDEX_MINIMUM_VALUE, ZONE_INDEX_MAXIMUM_VALUE } from './colorZone';
...
function toBuffer(obj: ColorZone) {
	...
	if (obj.count < ZONE_INDEX_MINIMUM_VALUE || obj.count > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid count value given for stateColorZone LIFX packet, must be a number between ' +
	...
	if (obj.index < ZONE_INDEX_MINIMUM_VALUE || obj.index > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid index value given for stateColorZone LIFX packet, must be a number between ' +
					ZONE_INDEX_MINIMUM_VALUE +
					' and ' +
					ZONE_INDEX_MAXIMUM_VALUE
			)
...
export const stateZone: PacketBodyHandler = {
	type: 503,
	name: 'stateColorZone',
	legacy: false,
	size: SIZE,
