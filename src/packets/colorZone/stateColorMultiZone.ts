...
import { MultiZone, ZONE_INDEX_MINIMUM_VALUE, ZONE_INDEX_MAXIMUM_VALUE } from './colorZone';
...
function toBuffer(obj: MultiZone) {
	...
	if (obj.count < ZONE_INDEX_MINIMUM_VALUE || obj.count > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid count value given for stateColorMultiZone LIFX packet, must be a number between ' +
	...
	if (obj.index < ZONE_INDEX_MINIMUM_VALUE || obj.index > ZONE_INDEX_MAXIMUM_VALUE) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid index value given for stateColorMultiZone LIFX packet, must be a number between ' +
					ZONE_INDEX_MINIMUM_VALUE +
					' and ' +
					ZONE_INDEX_MAXIMUM_VALUE
			)
	...
	if (obj.color.length < 1 || obj.color.length > 8) {
		throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
			.withContextualMessage(
				'Invalid set of color values given for stateColorMultiZone LIFX packet, must be an array of 1 to 8 objects'
			)
			.build();
	}
	...
	obj.color.forEach(function(colorObj, index) {
		if (colorObj.hue < HSBK_MINIMUM_RAW || colorObj.hue > HSBK_MAXIMUM_RAW) {
			throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
				.withContextualMessage(
					'Invalid color hue given at index ' +
						index +
		...
		if (colorObj.saturation < HSBK_MINIMUM_RAW || colorObj.saturation > HSBK_MAXIMUM_RAW) {
			throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
				.withContextualMessage(
					'Invalid color saturation given at index ' +
						index +
						', must be a number between ' +
						HSBK_MINIMUM_RAW +
						' and ' +
		...
		if (colorObj.brightness < HSBK_MINIMUM_RAW || colorObj.brightness > HSBK_MAXIMUM_RAW) {
			throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
				.withContextualMessage(
					'Invalid color brightness given at index ' +
						index +
						', must be a number between ' +
						HSBK_MINIMUM_RAW +
						' and ' +
		...
		if (colorObj.kelvin < HSBK_MINIMUM_KELVIN || colorObj.kelvin > HSBK_MAXIMUM_KELVIN) {
			throw new ServiceErrorBuilder(ER_LIGHT_COLOR_RANGE)
				.withContextualMessage(
					'Invalid color kelvin given at index ' +
						index +
						', must be a number between ' +
						HSBK_MINIMUM_RAW +
						' and ' +
...
export const stateMultiZone: PacketBodyHandler = {
	type: 506,
